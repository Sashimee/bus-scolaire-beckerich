...
| R11 | 13 | **L'intégration Google fonctionne jusqu'à l'écriture.** Deux obstacles levés le 2026-08-09 : la CI ne passait pas l'ID client, et l'échange de jeton se faisait sans `client_secret` — il passe désormais par le Worker. Reste le dernier : Google délivre un jeton **sans la portée `calendar.app.created`**, d'où un `403 insufficient authentication scopes` à la synchronisation. La portée doit être déclarée sur l'écran de consentement du projet Google — configuration extérieure au dépôt. L'application le détecte maintenant à la connexion et dit quoi faire. | Déclarer la portée, retirer l'accès de l'application dans le compte Google, se reconnecter, synchroniser. |
