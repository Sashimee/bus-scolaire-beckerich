...
| R49 | 29 | **À moitié levée le 2026-08-25.** La moitié qui se voyait est vérifiée : `https://dev.schoulbus.lu/api/sante` renvoie `push: true` et une `clePubliqueVapid` **identique** à la variable de dépôt `CLE_VAPID_DEV` inlinée dans l'image — les deux moitiés de la paire concordent donc, ce qui était le vrai risque. **Reste** qu'aucune notification n'a été délivrée à un vrai téléphone depuis cette pile. Ancien texte pour trace : **La pile `dev` n'a jamais reçu de notification push réelle.** Elle a sa propre paire VAPID ; la moitié publique doit être posée en variable de dépôt `CLE_VAPID_DEV` et se retrouver dans l'image, la moitié privée dans `VAPID_JWK` côté Dokploy. Si les deux ne concordent pas, l'abonnement est accepté et **rien n'arrive** — sans message d'erreur. | Après déploiement : s'abonner depuis `dev.schoulbus.lu` et publier un rappel d'essai. Vérifier l'onglet « Journal » de `/edition` sur la pile de dev. |
| ~~R50~~ | 29 | ~~La pile `dev` partage le Traefik et la machine de la production.~~ **Levée le 2026-08-25** au déploiement réel. Dokploy a nommé la pile `compose-index-neural-capacitor-cfbh54`, distincte de la production (`compose-hack-neural-pixel-ai3w3f`) : `docker volume ls` montre trois volumes séparés (`…capacitor-cfbh54_postgres`, `…pixel-ai3w3f_postgres` pour la prod, `bus-beckerich_postgres` pour la pile locale). La base de dev est bien **neuve et vide** — dix tables à zéro, `migration` à 5. Les routeurs suffixés cohabitent : `dev.schoulbus.lu` et `app.schoulbus.lu` répondent tous deux 200, chacun avec sa propre clé VAPID. Ancien texte pour trace : **La pile `dev` partage le Traefik et la machine de la production.** Les routeurs sont suffixés et les volumes préfixés par Dokploy, mais rien dans le dépôt ne l'impose : un `docker compose` lancé à la main sans `-p` viserait la mauvaise pile. La base de dev doit être **vide et distincte** — une collision de volume mettrait les données de production dans dev, ce qui ne se verrait pas tout de suite. | Avant le premier `up` : `docker volume ls` et vérifier qu'aucun volume de dev ne porte le préfixe de la pile de production. Après : `curl https://dev.schoulbus.lu/api/sante` et confirmer que les compteurs sont à zéro. |
...
### Le déploiement réel — 2026-08-25

Fait dans la foulée. DNS `dev.schoulbus.lu` → `57.131.136.250` (la même VPS), service Compose
**`bus-app-dev`** créé dans le projet Schoulbus (`composeId=JrHQ4zU5QlxKEmfSAjI2u`,
appName `compose-index-neural-capacitor-cfbh54`), `sourceType: raw` comme la production.

Ce qui est confirmé sur le vrai serveur :

- Les trois conteneurs tournent sur `ghcr.io/sashimee/bus-{api,site}:dev`, tous `healthy`.
  **C'est le premier déploiement du projet qui TIRE ses images de GHCR** au lieu de les
  construire sur la VPS — le chemin que R42 laissait à éprouver.
- Traefik a émis le certificat et route les deux hôtes : `dev.schoulbus.lu` et
  `app.schoulbus.lu` répondent 200, la production n'a pas bronché.
- La CSP servie par l'image de dev porte `https://dev.schoulbus.lu/api` — le site de dev
  n'appelle donc pas l'API de production.
- `/api/sante` : `base`, `push`, `comptes`, `rappels` au vert ; `courriel: false` (SMTP
  laissé vide, comme décidé) ; `origines: https://dev.schoulbus.lu`.
- Base **vide et distincte** (voir R50), et un premier compte amorcé par
  `creer-utilisateur.mjs`, dont la **connexion réelle a été exercée** — jeton rendu avec
  les six capacités.

**Où s'arrête la preuve** : R48 (rien vu sur un téléphone) et R49 (aucune notification
délivrée). R50 est levée.
