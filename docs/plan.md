...
| R45 | Consolidation | **Partiellement levée le 2026-09-07.** Un vrai compte a publié une perturbation depuis `/edition` sur `app.schoulbus.lu`, elle est arrivée dans l'application, et elle a été retirée — le chemin des capacités est donc éprouvé en production, et plus seulement par les tests. Deux enseignements au passage : la publication échouait d'abord sur une **session expirée**, refusée avant même d'être journalisée (d'où un journal vide qui laissait croire à une panne d'écriture), et le jeton mort survivait à son refus — corrigé le même jour. **Reste ouvert** : la migration `004-retrait-agents.sql` (`drop table agent_commune, agent_traduction`) n'a jamais été constatée appliquée sur la base de PRODUCTION ; elle n'a tourné que sur des schémas de test neufs. | Se connecter à la base de production et vérifier que `\dt` ne liste plus `agent_commune` ni `agent_traduction`. C'est tout ce qui reste de cette réserve. |
...
1. ~~**Éprouver le chemin avant de l'ouvrir.**~~ **Fait le 2026-09-07** : une
   ...
   R45, dont il ne reste que la vérification de la migration 004 en base.
