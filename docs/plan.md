...
| R11 | 13 | **L'échange de jeton Google se faisait depuis le navigateur, sans `client_secret`.** Google ne l'accepte que d'un client public ; pour un client « Application Web » il l'exige. Constaté en usage réel le 2026-08-09 : après autorisation, retour sur la page, aucun agenda créé, bouton de connexion toujours là — et **échec entièrement muet**. L'échange passe désormais par le Worker, qui détient le secret comme il détient déjà celui de GitHub, et tout refus est affiché avec le motif de Google. Reste à poser `GOOGLE_CLIENT_ID` et `GOOGLE_CLIENT_SECRET` côté Worker. | `npx wrangler secret put` pour les deux, puis reconnecter un compte depuis `/agenda`. |
