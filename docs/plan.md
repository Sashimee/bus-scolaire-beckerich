...
| R11 | 13 | **L'intégration Google n'a jamais tourné, et deux obstacles l'attendent.** (1) La CI ne passait pas `VITE_ID_CLIENT_GOOGLE` au build : poser la variable n'aurait rien changé. Corrigé le 2026-08-09. (2) `terminerConnexion` échange le code sans `client_secret` : Google l'accepte pour un client public (mobile, ordinateur), pas pour un client « application Web ». L'échange risque de répondre `invalid_client`. La parade tient à l'architecture déjà en place — faire passer l'échange par le Worker, qui détient déjà le secret GitHub. | Créer l'ID client, poser `ID_CLIENT_GOOGLE` en variable de dépôt, essayer. Si l'échange est refusé, déplacer l'échange dans le Worker. |
